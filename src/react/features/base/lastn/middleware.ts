...
// Mobile import removed for web-only build
import {
    SET_CAR_MODE,
...
const _updateLastN = debounce(({ dispatch, getState }: IStore) => {
    ...
    // const { appState } = state['features/mobile/background'] || {}; // Mobile feature removed
    const appState = undefined; // Mobile background state removed for web-only build
...
MiddlewareRegistry.register(store => next => action => {
    ...
    switch (action.type) {
    // case APP_STATE_CHANGED: // Mobile action removed for web-only build
